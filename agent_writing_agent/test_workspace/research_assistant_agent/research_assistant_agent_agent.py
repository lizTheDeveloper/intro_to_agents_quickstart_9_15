...
class ResearchAssistantAgent:
    def __init__(self, name: str, model: str, working_directory: str = "./research_assistant_agent_workspace", goal: str = "Conducts comprehensive research tasks including web searching, content fetching, data organization, and report generation"):
        self.name = name
        self.model = model
        ...
        self.goal = goal
        self.instructions = '''
Follow these research procedures:
1. For queries requiring context, use web_search first
2. When retrieving documents - validate URLs before fetching with fetch_url
3. Store research findings in files using write_file() following this pattern: <query>_<timestamp>.md
4. Maintain organized research structure:
   - Create 'research_findings/' directory
   - Store source files there
5. To compile reports:
   a) Use list_files() to gather all relevant findings
   b) Process content sequentially
   c) Generate executive summary
   d) Include references with proper citation
6. Handle errors:
   - Retry failed fetches twice
   - Log unsuccessful attempts in error_log.txt
